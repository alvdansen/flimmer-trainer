"""CLI for Flimmer video tools.

Usage:
    python -m flimmer.video scan <dir>                  — probe + validate + report
    python -m flimmer.video ingest <video|dir> --output <dir>  — scene detect + split + normalize
    python -m flimmer.video normalize <dir> --output <dir> — batch normalize pre-cut clips
    python -m flimmer.video caption <dir> --provider gemini — caption clips via VLM
    python -m flimmer.video audit <dir> --provider gemini   — audit existing captions
    python -m flimmer.video score <dir>                 — score caption quality (no API needed)
    python -m flimmer.video extract <dir> --output <dir>   — extract reference images from clips
    python -m flimmer.video triage <dir> --concepts <dir>  — match clips against reference images

All commands accept --config <path> to load a flimmer_data.yaml config file.
...
def cmd_scan(args: argparse.Namespace) -> None:
    ...
    from flimmer.video.probe import probe_directory
...
def _ingest_single_video(
    ...
    """
    from flimmer.video.probe import probe_video
    ...
    if triage_segments is not None:
        ...
        from flimmer.video.split import split_video_segments
        ...
        from flimmer.video.scene import detect_scenes
        from flimmer.video.split import split_video_at_scenes
...
def cmd_normalize(args: argparse.Namespace) -> None:
    ...
    from flimmer.video.split import normalize_directory
...
def cmd_score(args: argparse.Namespace) -> None:
    ...
    from flimmer.caption.scoring import ScoringConfig, format_score_report, score_directory
...
def cmd_extract(args: argparse.Namespace) -> None:
    ...
    from flimmer.video.extract import (
        extract_directory,
        extract_from_selections,
        generate_selection_template,
    )
    from flimmer.video.extract_models import ExtractionConfig, ExtractionStrategy
...
def cmd_triage(args: argparse.Namespace) -> None:
    ...
    from flimmer.triage import organize_clips, triage_clips
    from flimmer.triage.models import VideoTriageReport
    ...
    if args.organize:
        if isinstance(report, VideoTriageReport):
            ...
            print(f"  python -m flimmer.video ingest {directory} -o <output> "
...
def cmd_audit(args: argparse.Namespace) -> None:
    ...
    from flimmer.caption import audit_captions
...
def _caption_clips(directory: Path, args: argparse.Namespace) -> None:
    ...
    from flimmer.caption import caption_clips
    from flimmer.caption.models import CaptionConfig
...
def _load_video_config(config_path: str | None, fps_override: int | None = None):
    """Load VideoConfig from a flimmer_data.yaml, or use defaults.

    Args:
        config_path: Optional path to flimmer_data.yaml.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="flimmer.video",
        description="Flimmer video ingestion and scene detection tools",
    )
    ...
    scan_parser.add_argument(
        "--config", "-c",
        help="Path to flimmer_data.yaml (default: use standard Wan defaults)",
    )
    scan_parser.add_argument(
        "--fps", type=int,
    ...
    ingest_parser.add_argument(
        "--config", "-c",
        help="Path to flimmer_data.yaml",
    )
    ingest_parser.add_argument(
...
def main() -> None:
    """Entry point for python -m flimmer.video."""
    parser = build_parser()
