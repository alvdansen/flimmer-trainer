...
Usage:
    from flimmer.video.extract import extract_directory, ExtractionConfig

    config = ExtractionConfig(strategy="first_frame")
    report = extract_directory("clips/", "refs/", config)

CLI:
    python -m flimmer.video extract clips/ --output refs/
    python -m flimmer.video extract clips/ --output refs/ --strategy best_frame
    python -m flimmer.video extract clips/ --template selections.json
"""
...
from flimmer.video.errors import ExtractionError, FFmpegNotFoundError
...
from flimmer.video.image_quality import compute_sharpness, is_blank
...
def generate_selection_template(
    ...
    print(f"  python -m flimmer.video extract {source_dir} --output <dir> --selections {output_path}")
