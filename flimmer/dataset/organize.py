...
Flow:
    ...
    3. Build output layout (flat or flimmer)
...
from flimmer.video.models import Severity
...
def _build_flimmer_paths(
    sample: SamplePair,
    output_dir: Path,
    used_stems: set[str],
) -> tuple[Path, Path | None, Path | None]:
    """Compute destination paths for flimmer hierarchical layout.
...
def organize_dataset(
    ...
    Args:
        ...
        layout: FLAT (default) or FLIMMER hierarchical structure.
    ...
    if not valid_samples:
        raise OrganizeError(
            ...
            + ".\n"
            f"Run 'python -m flimmer.dataset validate {source_dir}' to see issues."
        )

    # Build paths and transfer files
    used_stems: set[str] = set()
    path_builder = _build_flimmer_paths if layout == OrganizeLayout.FLIMMER else _build_flat_paths
    ...
    if trainers:
        from flimmer.dataset.trainers import generate_trainer_config
