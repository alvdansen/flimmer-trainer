...
Usage:
    python -m flimmer.dataset validate <path>
    python -m flimmer.dataset validate <path> --manifest
    python -m flimmer.dataset validate <path> --buckets
    python -m flimmer.dataset validate <path> --quality --duplicates
    python -m flimmer.dataset validate <path> --json
    python -m flimmer.dataset organize <path> -o <output>
    python -m flimmer.dataset organize <path> -o <output> -t musubi
    python -m flimmer.dataset organize <path> -o <output> -l flimmer --manifest
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m flimmer.dataset",
        description="Flimmer dataset validation and organization tools.",
    )
    ...
    validate.add_argument(
        "path",
        help="Path to the dataset folder (or a flimmer_data.yaml config file).",
    )
    validate.add_argument(
        "--config",
        help="Path to a flimmer_data.yaml config file. If not provided, uses defaults.",
    )
    validate.add_argument(
        "--manifest",
        action="store_true",
        help="Write a flimmer_manifest.json file to the dataset folder.",
    )
    validate.add_argument(
        "--buckets",
        action="store_true",
    ...
    organize.add_argument(
        "--layout", "-l",
        choices=["flat", "flimmer"],
        default="flat",
        help="Output layout: flat (default, universal) or flimmer (hierarchical).",
    )
    organize.add_argument(
        "--trainer", "-t",
        action="append",
    ...
    organize.add_argument(
        "--config", "-c",
        help="Path to flimmer_data.yaml config file.",
    )
    organize.add_argument(
        "--manifest",
        action="store_true",
        help="Write flimmer_manifest.json to output directory.",
    )
...
def _format_validate_hint(dataset_path: Path) -> str:
    ...
    return (
        ...
        f"  python -m flimmer.dataset organize {path_str} -o <output_dir>\n"
        f"  python -m flimmer.dataset organize {path_str} -o <output_dir> -t musubi\n"
        f"  python -m flimmer.dataset organize {path_str} -o <output_dir> -t aitoolkit\n"
    )


def cmd_validate(args: argparse.Namespace) -> int:
    """Run the validate command."""
    from flimmer.config.data_schema import FlimmerDataConfig, QualityConfig
    from flimmer.dataset.manifest import build_manifest, write_manifest
    ...
    from flimmer.dataset.validate import validate_all
    ...
    # Write manifest
    if args.manifest:
        manifest_path = dataset_path / "flimmer_manifest.json"
        if not dataset_path.is_dir():
            manifest_path = dataset_path.parent / "flimmer_manifest.json"
...
def cmd_organize(args: argparse.Namespace) -> int:
    ...
    from flimmer.dataset.report import format_organize_plaintext, print_organize_report

    source_path = Path(args.path).resolve()
    output_path = Path(args.output).resolve()

    # Layout
    layout = OrganizeLayout.FLIMMER if args.layout == "flimmer" else OrganizeLayout.FLAT

    # Config
    config: FlimmerDataConfig | None = None
    if args.config:
        from flimmer.config.loader import load_config
    ...
    if args.manifest and not args.dry_run:
        from flimmer.dataset.manifest import write_manifest
        from flimmer.dataset.validate import validate_all

        if config is None:
            config = FlimmerDataConfig(datasets=[{"path": str(source_path)}])
        report = validate_all(config, config_dir=source_path)
        manifest_path = output_path / "flimmer_manifest.json"
        write_manifest(report, config, manifest_path)
        print(f"\nManifest written to: {manifest_path}")
