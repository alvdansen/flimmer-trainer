...
Follows the same pattern as flimmer.video.validate:
...
Usage:
    ...
    config = load_config("flimmer_data.yaml")
        ...
        try:
            ...
            if img is None:
                ...
                if exposure_range is not None:
                    from flimmer.dataset.quality import compute_exposure
...
def validate_dataset(
    ...
    if motion_config.min_intensity is not None or motion_config.max_intensity is not None:
        try:
            from flimmer.dataset.quality import compute_motion_intensity
    ...
    if config.quality.check_duplicates:
        ...
        if len(ref_paths) >= 2:
            try:
                from flimmer.dataset.quality import find_duplicates
