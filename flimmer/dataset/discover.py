...
Scans directories, detects layout (flat vs flimmer), classifies files by
...
from flimmer.video.models import IssueCode, Severity, ValidationIssue
...
# Extensions recognized as images (targets or references)
...
def detect_structure(directory: Path) -> StructureType:
    """Detect whether a directory uses flat or flimmer layout.

    Flimmer layout has a `training/targets/` subdirectory.
    ...
    Returns:
        StructureType.FLIMMER if training/targets/ exists,
        StructureType.FLAT otherwise.
    """
    targets_dir = directory / "training" / "targets"
    if targets_dir.is_dir():
        return StructureType.FLIMMER
        ...
        if caption_path is not None:
            ...
            issues.append(ValidationIssue(
                ...
                message=(
                    ...
                    f"Generate captions with: python -m flimmer.video caption"
                ),
                field="caption",
            ))
        ...
        if reference_path is not None:
            ...
            issues.append(ValidationIssue(
                ...
                message=(
                    ...
                    f"Extract references with: python -m flimmer.video extract"
                ),
                field="reference",
            ))
...
def discover_dataset(
    ...
    if not samples:
        dataset_issues.append(ValidationIssue(
            ...
            severity=Severity.ERROR,
            message=(
                f"No target files found in '{directory}'. "
                f"Expected video or image files "
                f"({', '.join(sorted(VIDEO_EXTENSIONS)[:3])}, "
                f"{', '.join(sorted(IMAGE_EXTENSIONS)[:3])}...) "
                f"in {'training/targets/' if structure == StructureType.FLIMMER else 'the directory'}."
            ),
            field="dataset",
        ))
