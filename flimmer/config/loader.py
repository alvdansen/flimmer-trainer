"""Config loader for Flimmer data configs.
...
consuming a Flimmer data config.

Usage::

    from flimmer.config import load_data_config

    # From a YAML file
    config = load_data_config("path/to/flimmer_data.yaml")

    # From a directory (looks for flimmer_data.yaml inside)
...
CONFIG_FILENAME = "flimmer_data.yaml"


def load_data_config(path: str | Path) -> FlimmerDataConfig:
    """Load and validate a Flimmer data config.

    This is the main entry point. It accepts three kinds of input:

    1. **A YAML file path**: loads and parses it directly.
    2. **A directory containing flimmer_data.yaml**: discovers and loads it.
    ...
    Returns:
        A fully validated FlimmerDataConfig with all defaults applied
        and all relative paths resolved.

    Raises:
        FileNotFoundError: If the path doesn't exist.
        FlimmerConfigError: If the config has validation errors.
    """
    ...
    try:
        config = FlimmerDataConfig.model_validate(config_data)
    except ValidationError as e:
        raise FlimmerConfigError(_format_validation_error(e)) from e
...
class FlimmerConfigError(Exception):
    """Raised when a Flimmer data config has validation errors.
...
def _check_dataset_paths(config: FlimmerDataConfig) -> None:
...
def _format_validation_error(error: ValidationError) -> str:
    ...
    lines.append(
        "\nSee config_templates/ in flimmer-trainer for valid config files."
    )
    return "\n".join(lines)
