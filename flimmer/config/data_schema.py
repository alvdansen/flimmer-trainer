"""Pydantic v2 models for the Flimmer data config.

This is the schema that describes a dataset to Flimmer. It answers:
"What files do I have, how should they be handled, and what quality
standards should they meet?"

The data config lives with the dataset (typically as flimmer_data.yaml)
...
from flimmer.config.defaults import (
    UMT5_MAX_TOKENS,
...
class FlimmerDataConfig(BaseModel):
    """Root config model for a Flimmer dataset.

    This is the complete schema for flimmer_data.yaml. Minimum viable
