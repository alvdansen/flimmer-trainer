"""Tests for flimmer.dataset.trainers — trainer config generators.
...
from flimmer.config.data_schema import FlimmerDataConfig, VideoConfig
from flimmer.dataset.errors import OrganizeError
from flimmer.dataset.models import OrganizeLayout, OrganizedSample
...
class TestHelpers:
    ...
    def test_video_directory_flimmer(self, tmp_path: Path):
        result = _video_directory(tmp_path, OrganizeLayout.FLIMMER)
...
class TestMusubiConfig:
    ...
    def _default_config(self) -> FlimmerDataConfig:
    ...
    def test_toml_flimmer_layout_path(self, tmp_path: Path):
        """Flimmer layout points video_directory at training/targets/."""
        path = generate_trainer_config(
            "musubi", self._make_samples(), tmp_path,
            self._default_config(), OrganizeLayout.FLIMMER,
        )
        content = path.read_text(encoding="utf-8")
