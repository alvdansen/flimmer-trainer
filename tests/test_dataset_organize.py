"""Tests for flimmer.dataset.organize — core organize engine.

Tests cover: flat layout, flimmer layout, copy vs move, dry-run,
...
def _default_config(path: str = ".") -> FlimmerDataConfig:
...
class TestOrganizeModels:
    def test_organize_layout_values(self):
        ...
        assert OrganizeLayout.FLIMMER.value == "flimmer"
...
class TestOrganizeErrors:
    def test_organize_error_hierarchy(self):
        from flimmer.dataset.errors import FlimmerDatasetError
        err = OrganizeError("test")
        assert isinstance(err, FlimmerDatasetError)
...
# ---------------------------------------------------------------------------
# Flimmer layout organize
# ---------------------------------------------------------------------------

class TestFlimmerOrganize:
    def test_basic_flimmer(self, tmp_path: Path):
        """Flimmer layout creates training/targets/ and training/signals/."""
        src = _make_flat_dataset(tmp_path / "src", ["a", "b"])
        out = tmp_path / "out"

        result = organize_dataset(src, out, layout=OrganizeLayout.FLIMMER)

        assert result.layout == OrganizeLayout.FLIMMER
