"""Tests for the Flimmer data config schema, loader, and validation.
...
from flimmer.config.defaults import UMT5_MAX_TOKENS, WAN_TRAINING_FPS
from flimmer.config.loader import FlimmerConfigError, load_data_config
...
def write_config(path: Path, data: dict) -> Path:
    ...
    config_path = path / "flimmer_data.yaml"
...
class TestMinimalConfig:
    ...
    def test_discover_config_in_directory(self, tmp_path: Path, tmp_dataset: Path):
        """Loading a directory finds flimmer_data.yaml inside it."""
...
class TestValidationErrors:
    ...
    def test_invalid_frame_count(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="4n.1"):
    ...
    def test_invalid_frame_count_suggests_nearest(
        ...
        with pytest.raises(FlimmerConfigError, match="77.*81"):
            load_data_config(config_path)
    ...
    def test_invalid_resolution(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="480.*720"):
            load_data_config(config_path)
    ...
    def test_invalid_use_case(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="character.*motion.*object.*style"):
            load_data_config(config_path)
    ...
    def test_invalid_text_format(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="jsonl.*txt"):
            load_data_config(config_path)
    ...
    def test_invalid_upscale_policy(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="never.*warn"):
            load_data_config(config_path)
    ...
    def test_invalid_sar_policy(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="auto_correct.*reject"):
            load_data_config(config_path)
    ...
    def test_invalid_token_dropout_rate_too_high(
        ...
        with pytest.raises(FlimmerConfigError, match="token_dropout_rate"):
            load_data_config(config_path)
    ...
    def test_invalid_loss_multiplier_zero(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="loss_multiplier"):
            load_data_config(config_path)
    ...
    def test_invalid_repeats_zero(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="repeats"):
            load_data_config(config_path)
    ...
    def test_missing_dataset_path(self, tmp_path: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="No dataset paths"):
            load_data_config(config_path)
    ...
    def test_nonexistent_dataset_path(self, tmp_path: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="not found"):
            load_data_config(config_path)
    ...
    def test_invalid_reference_source(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="first_frame.*folder.*none"):
            load_data_config(config_path)
    ...
    def test_invalid_bucketing_dimension(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="color"):
            load_data_config(config_path)
    ...
    def test_invalid_downscale_method(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(
            FlimmerConfigError, match="area.*bicubic.*bilinear.*lanczos"
        ):
            load_data_config(config_path)
    ...
    def test_invalid_fps_zero(self, tmp_path: Path, tmp_dataset: Path):
        ...
        with pytest.raises(FlimmerConfigError, match="fps"):
            load_data_config(config_path)
...
class TestEdgeCases:
    ...
    def test_empty_yaml_file(self, tmp_path: Path):
        ...
        config_path = tmp_path / "flimmer_data.yaml"
