"""Tests for flimmer.dataset.discover — file discovery and structure detection.
...
from flimmer.dataset.errors import DatasetValidationError
from flimmer.dataset.models import StructureType
from flimmer.video.models import IssueCode, Severity
...
def _make_flimmer_dataset(tmp_path: Path, stems: list[str]) -> Path:
    """Create a flimmer-structured dataset."""
    for stem in stems:
...
class TestDetectStructure:
    ...
    def test_flimmer_with_training_targets(self, tmp_path: Path):
        (tmp_path / "training" / "targets").mkdir(parents=True)
        assert detect_structure(tmp_path) == StructureType.FLIMMER

    def test_flimmer_needs_both_dirs(self, tmp_path: Path):
        """Just training/ alone doesn't make it flimmer — needs training/targets/."""
...
class TestDiscoverFiles:
    ...
    def test_flimmer_targets(self, tmp_path: Path):
        _make_flimmer_dataset(tmp_path, ["clip_001"])
        files = discover_files(tmp_path, StructureType.FLIMMER)
        assert len(files["targets"]) == 1
        assert files["targets"][0].name == "clip_001.mp4"

    def test_flimmer_captions(self, tmp_path: Path):
        _make_flimmer_dataset(tmp_path, ["clip_001"])
        files = discover_files(tmp_path, StructureType.FLIMMER)
        assert len(files["captions"]) == 1
        assert files["captions"][0].name == "clip_001.txt"

    def test_flimmer_references(self, tmp_path: Path):
        _make_flimmer_dataset(tmp_path, ["clip_001"])
        files = discover_files(tmp_path, StructureType.FLIMMER)
        assert len(files["references"]) == 1

    def test_flimmer_image_targets(self, tmp_path: Path):
        """Image files (.png) in training/targets/ are discovered as targets."""
        for stem in ["img_001", "img_002", "img_003"]:
            _touch(tmp_path / "training" / "targets" / f"{stem}.png")
            _touch(tmp_path / "training" / "signals" / "captions" / f"{stem}.txt", b"Caption")
        files = discover_files(tmp_path, StructureType.FLIMMER)
        assert len(files["targets"]) == 3
        assert len(files["other"]) == 0

    def test_flimmer_mixed_targets(self, tmp_path: Path):
        """Mixed video (.mp4) + image (.png) targets are all discovered."""
        for stem in ["vid_001", "vid_002"]:
            _touch(tmp_path / "training" / "targets" / f"{stem}.mp4")
            _touch(tmp_path / "training" / "signals" / "captions" / f"{stem}.txt", b"Caption")
    ...
    def test_flimmer_other_in_targets_dir(self, tmp_path: Path):
        """Non-media files in targets/ are classified as other."""
        _make_flimmer_dataset(tmp_path, ["clip_001"])
...
class TestValidateFileType:
    def test_returns_none_without_filetype(self, tmp_path: Path, monkeypatch):
        ...
        import flimmer.dataset.discover as mod
...
class TestDiscoverDataset:
    ...
    def test_flimmer_dataset(self, tmp_path: Path):
        _make_flimmer_dataset(tmp_path, ["clip_001", "clip_002"])
        config = _default_config()
        result = discover_dataset(tmp_path, config)
        assert result.structure == StructureType.FLIMMER
