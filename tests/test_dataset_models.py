"""Tests for flimmer.dataset.models and related issue codes.
...
from flimmer.config.data_schema import QualityConfig
from flimmer.dataset.errors import DatasetValidationError, FlimmerDatasetError
...
class TestStructureType:
    ...
    def test_flimmer_value(self):
        assert StructureType.FLIMMER == "flimmer"

    def test_from_string(self):
        assert StructureType("flat") == StructureType.FLAT
        assert StructureType("flimmer") == StructureType.FLIMMER
...
class TestDatasetValidation:
    ...
    def test_some_invalid(self):
        ...
        dv = DatasetValidation(
            source_path=Path("/data"),
            structure=StructureType.FLIMMER,
            samples=[
    ...
    def test_frozen(self):
        ...
        with pytest.raises(ValidationError):
            dv.structure = StructureType.FLIMMER  # type: ignore[misc]
...
class TestErrors:
    def test_dataset_error_is_exception(self):
        assert issubclass(FlimmerDatasetError, Exception)

    def test_validation_error_inherits(self):
        assert issubclass(DatasetValidationError, FlimmerDatasetError)
